...
class EvaluacionMatriz {
    ...
    public function db_get_evaluaciones_by_run_profesor_and_id_curso_and_mes_actual($run_profesor, $id_curso, $mes_actual){
        global $myPDO;
        $sentencia = $myPDO->prepare("CALL get_evaluaciones_by_run_profesor_and_id_curso_and_mes_actual(?,?,?)");
        $sentencia->bindParam(1, $run_profesor, \PDO::PARAM_STR, 9);
        $sentencia->bindParam(2, $id_curso, \PDO::PARAM_INT);
        $sentencia->bindParam(3, $mes_actual, \PDO::PARAM_INT);
        $sentencia->execute();
    ...
    public function db_get_evaluaciones_by_id_curso($id_curso){
        global $myPDO;
        $sentencia = $myPDO->prepare("CALL get_evaluaciones_by_id_curso(?)");
        $sentencia->bindParam(1, $id_curso, \PDO::PARAM_INT);
        $sentencia->execute();
