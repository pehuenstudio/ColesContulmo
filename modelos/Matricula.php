...
class Matricula {
    ...
    public function db_get_matricula_by_run_alumno_and_rbd_esta_and_periodo(){
        global $myPDO;
        $sentencia = $myPDO->prepare("CALL get_matricula_by_run_alumno_and_rbd_esta_and_periodo(?,?,?)");
        $sentencia->bindParam(1, $this->run_alumno, \PDO::PARAM_STR, 9);
        $sentencia->bindParam(2, $this->rbd_establecimiento, \PDO::PARAM_INT);
        $sentencia->bindParam(3, $this->periodo, \PDO::PARAM_INT);
        $sentencia->execute();
    ...
    public function db_ins_matricula(){
        global $myPDO;
        $sentencia = $myPDO->prepare("CALL ins_matricula(?,?,?,?,?,?,?,?)");
